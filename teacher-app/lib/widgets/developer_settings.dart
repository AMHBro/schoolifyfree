...
class _DeveloperSettingsState extends State<DeveloperSettings> {
  ...
  Future<void> _saveSettings() async {
    ...
    if (_useCustomUrl && _urlController.text.isNotEmpty) {
      if (!_urlController.text.contains('railway.app')) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
                'Only Railway URLs are allowed. Please use a railway.app URL.',
              ),
  ...
  Widget _buildUrlOption(String title, String url, {bool isCustom = false}) {
    final isSelected = isCustom
        ? _useCustomUrl
        : (!_useCustomUrl && AppConfig.baseUrl == url);
                  ...
                  children: [
                    ...
                    TextField(
                      ...
                      decoration: const InputDecoration(
                        hintText:
                            'Enter Railway API URL (must contain railway.app)',
                        border: OutlineInputBorder(),
                        helperText: 'Only Railway URLs are allowed',
                      ),
                      onChanged: (value) {
                        setState(() {
