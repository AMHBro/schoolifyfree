...
async function createSampleTeacher() {
  ...
  try {
    ...
    // Try to find any existing subject for this stage
    ...
    if (existingTeacher) {
      ...
      // Let's check if associations exist
      const stageAssoc = await prisma.teacherStage.findFirst({
        where: { teacherId: existingTeacher.id, stageId: stage.id },
      });

      const subjectAssoc = await prisma.teacherSubject.findFirst({
        where: { teacherId: existingTeacher.id, subjectId: subject.id },
      });

      if (!stageAssoc) {
        console.log("🔗 Adding stage association...");
        await prisma.teacherStage.create({
          data: { teacherId: existingTeacher.id, stageId: stage.id },
        });
      }

      if (!subjectAssoc) {
        console.log("🔗 Adding subject association...");
        await prisma.teacherSubject.create({
          data: { teacherId: existingTeacher.id, subjectId: subject.id },
        });
      }
